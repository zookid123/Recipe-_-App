...
데이터는 신뢰도가 높고 무료로 제공되는 농림축산식품부 공공 데이터를 기반으로 했고, 타겟은 크게 세 그룹으로 나눴습니다.
자취생은 남은 재료로 뭐 해먹지라는 고민을 가장 많이 하는 그룹으로, 냉장고 파먹기와 유통기한 알림 기능으로 해결했고,
요리 초보는 쉽고 빠른 레시피가 필요한 그룹으로, 조리 전용 모드와 난이도 필터를 제공했으며,
요리 마니아는 레시피를 공유하고 피드백을 주고받고 싶은 그룹으로, 레시피 직접 작성과 커뮤니티 기능을 구현했습니다.
-->
...
| **커뮤니티** | 게시글(4카테고리), 댓글·대댓글, 좋아요, 1:1 채팅 |
...
| 항목 | 선택 | 주요 대안 탈락 이유 |
|------|------|-------------------|
| **프레임워크** | Flutter | React Native — Firebase 공식 지원 부족 / 네이티브 단독 — Android·iOS 별도 개발 필요 |
| **백엔드** | Firebase | 자체 서버 — 개발·운영 비용 과다, WebSocket 별도 구축 필요 / Supabase — Firebase 대비 Flutter 레퍼런스·커뮤니티 부족 |
...
프레임워크는 Flutter(플러터)를 선택했는데, Android(안드로이드)와 iOS를 코드 하나로 지원해야 했고, Firebase 팀이 직접 관리하는 공식 패키지가 있어서 연동이 가장 간편합니다. React Native(리액트 네이티브)도 고려했지만 Firebase 공식 지원이 부족해서 제외했습니다.
...
상태 관리는 별도 라이브러리 없이 스테이트풀위젯과 스트림빌더를 사용했는데, Firestore 스트림을 직접 소비할 수 있어서 추가 레이어 없이 실시간 데이터를 바로 화면에 반영할 수 있었고, 앱 전체에서 공유해야 하는 전역 상태가 로그인 정보 하나뿐이라 Riverpod(리버팟)이나 Bloc(블록) 같은 복잡한 라이브러리가 필요하지 않았습니다.
-->
...
홈 탭에서는 검색, 냉장고 관리, 냉장고 파먹기 기능을 바로 사용할 수 있어 앱의 핵심 기능으로 빠르게 진입할 수 있는 허브 역할을 하고,
레시피 탭에서는 상세보기, 작성, 조리 전용 모드, 북마크, 최근 본 레시피로 이동할 수 있으며 요리 기능에 중점을 둔 탭입니다.
그리고 커뮤니티 탭에서는 게시글 작성·조회, 1:1 채팅, 사용자 프로필로 이동할 수 있어 사용자들이 요리 경험을 나누는 소통 공간이고,
마이페이지 탭에서는 업적·칭호 확인, 알림 설정, 활동 내역까지 내 정보를 한곳에서 관리할 수 있습니다.
-->
...
[대본]
파일 구조입니다.
lib(립) 폴더 아래 screens(스크린스), services(서비스), widgets(위젯), constants(컨스턴츠) 네 폴더로 나뉘어 있고, screens(스크린스) 폴더에는 총 29개의 화면 파일이 있습니다.
services(서비스) 폴더의 auth_service(어스 서비스)가 로그인 상태를, comment_watcher(코멘트 왓처)가 알림 감시를 담당하고, constants(컨스턴츠) 폴더에는 냉장고 빠른 선택용 재료 목록이 정의되어 있습니다.
그리고 위젯 폴더에는 여러 화면에서 함께 쓰는 카드 위젯들이 들어 있습니다.
-->
...
[대본]
데이터 흐름은 크게 두 단계입니다.
먼저 관리자가 레시피 동기화 스크립트를 실행해서 농림축산식품부 API 데이터를 Firestore(파이어스토어)에 저장하는데, 앱에서 직접 API를 호출하지 않는 이유는 페이지네이션 처리 중 데이터 누락 문제가 있어서 관리자 스크립트로 한 번에 동기화하는 방식을 선택했습니다.
그 다음으로 Flutter(플러터) 앱은 Firestore(파이어스토어)를 스트림빌더로 실시간 구독해서 데이터가 바뀌는 순간 화면이 자동으로 갱신되고, 채팅, 알림, 레시피 목록 모두 이 방식으로 실시간 반영됩니다.
-->
...
레시피 컬렉션에는 공공 API 데이터와 사용자가 직접 작성한 레시피가 함께 저장되어 있어서 별도 구분 없이 통합 검색이 가능하고,
사용자 컬렉션은 북마크, 최근 본 레시피, 좋아요, 냉장고 재료, 알림까지 개인화 데이터가 모두 users 하위에 집중되어 있어 사용자별 데이터 접근이 간단합니다.
이런 서브컬렉션 계층 구조 덕분에 특정 레시피의 댓글만, 특정 사용자의 북마크만 쿼리할 수 있어 불필요한 데이터 로딩 없이 효율적으로 동작합니다.
